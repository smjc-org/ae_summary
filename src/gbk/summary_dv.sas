/*
 * Macro Name:              aummary_dv
 * Macro Purpose:           方案偏离汇总
 * Macro Documentation URL: https://github.com/smjc-org/macro_tfl/docs/summary_dv.md
 * Author:                  wtwang
 * Version Date:            2025-07-07
*/

%macro summary_dv(indata,
                  outdata,
                  dvseq                   = dvseq,
                  dvsev                   = dvsev,
                  dvsevn                  = dvsevn,
                  dvtyp                   = dvtyp,
                  dvtyp_other_var         = #null,
                  dvtyp_other_value       = %str(其他),
                  dvtyp_other_mode        = #auto,
                  usubjid                 = usubjid,
                  arm                     = #null,
                  arm_by                  = %nrstr(&arm),
     ...
     *  dvseq:                  变量-方案偏离序号
     *  dvsev:                  变量-方案偏离严重程度
     *  dvsevn:                 变量-方案偏离严重程度的数值表示
     *  dvtyp:                  变量-方案偏离类型
     *  dvtyp_other_var:        变量-方案偏离类型-其他类型，如果传入 #null，表示无此变量
     *  dvtyp_other_value:      变量-方案偏离类型为“其他”的类型名称
     *  dvtyp_other_mode:       变量-方案偏离类型为“其他”时，其他方案偏离类型的输出模式，可选 #auto, #overall, #detail
     *  usubjid:                变量-受试者唯一编号
    ...
    */
    ...
    %let dvsev                   = %upcase(%sysfunc(strip(%bquote(&dvsev))));
